...
int first_part_2023_14() {
    std::cout << std::transform_reduce(std::istream_iterator<std::string>(INPUT_SOURCE),
                                       std::istream_iterator<std::string>(), 0ull, std::plus<>(),
                                       [weights = std::vector<size_t>(), current_line_weight = 0ull](
...
void move(std::vector<std::string> &matrix) {
    ...
    for (int64_t y = Direction != DOWN ? 0 : ssize(matrix) - 1;
         Direction != DOWN ? y < ssize(matrix) : y >= 0; Direction != DOWN ? ++y : --y) {
        for (int64_t x = Direction != RIGHT ? 0 : ssize(matrix[0]) - 1;
             Direction != RIGHT ? x < ssize(matrix[0]) : x >= 0; Direction != RIGHT ? ++x : --x) {
            if (matrix[y][x] == '#')
                target_new[Direction % 2 == 0 ? x : y] = (Direction % 2 == 0 ? y : x) + direction_movement;
            if (matrix[y][x] == 'O')
                std::swap(matrix[y][x], Direction % 2 == 0 ? matrix[(target_new[x] += direction_movement) -
                                                                    direction_movement][x] : matrix[y][
                                                (target_new[y] += direction_movement) - direction_movement]);
        }
    }
}
...
int second_part_2023_14() {
    ...
    for (; i < 1000; ++i) {
        apply_cycle(matrix);
        auto res = static_cast<int64_t>(calculate_north_weight(matrix));
        results.push_back(res);

        if (results.size() > 10) {
            if (auto it = std::search(results.begin(), std::prev(results.end(), 10), std::prev(results.end(), 10),
                                      results.end()); it != std::prev(results.end(), 10)) {
